...
class minimal_sitemap{
			...
			if($sitemap_query->have_posts())
			{
				...
				while ($sitemap_query->have_posts())
				{
					...
					<?php if(has_post_thumbnail()): ?>
						<image:image>
							<image:loc><?php echo esc_url(get_the_post_thumbnail_url(get_the_ID(),'full')); ?></image:loc>
						</image:image>
					<?php endif; ?>
					
					<changefreq><?php echo esc_html(minimal_sitemap::changefreq()); ?></changefreq>
					<mobile:mobile/>
					</url>
					<?php
				}
	...
	public static function changefreq()
	{
		$output = 'weekly';
